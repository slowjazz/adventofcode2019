...
wires = inp.split()
inp = [[x.strip() for x in w.split(',')] for w in wires]
visited = {} # coords
directions = {'R':(1,0), 'L':(-1,0), 'U':(0,1), 'D':(0,-1)}
cur = (0,0)
cross = []
tsteps = 0
for t, w in enumerate(inp):
    tsteps=0
    for i in w:
        ...
        for s in range(1, steps+1):
            visit = (cur[0] + s*directions[dir][0], cur[1] + s*directions[dir][1])
            tsteps +=1
            if visit in visited:
                if visited[visit][0] == abs(1-t):
                    cross.append((visit, visited[visit][1] + tsteps))
            else:
                visited[visit] = [t, tsteps]
            #print(visit, cross)
        cur = visit
    cur = (0,0)
...
bestdist = float('inf')
# for c in cross:
#     if abs(c[0]) + abs(c[1]) < bestdist:
#         bestPoint = c
#         bestdist = abs(c[0]) + abs(c[1])
# if bestdist== float('inf'):
#     print('NO CROSS')
# print(bestPoint)
# print(bestdist)
print(min(cross, key=lambda x: x[1]))